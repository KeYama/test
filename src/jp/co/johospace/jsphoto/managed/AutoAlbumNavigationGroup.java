...
import jp.co.johospace.jsphoto.AutoAlbumListActivity;
import jp.co.johospace.jsphoto.OnlineSchedulerSetupActivity;
...
public class AutoAlbumNavigationGroup extends NavigationGroup {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.auto_album_navigation_group);
...
//				e.printStackTrace();		/*$debug$*/	// TODO $ol
		...
		protected void onPostExecute(List<AuthPreference> result) {
			...
			if (result != null) {
				...
				for (AuthPreference pref : result) {
					if (ClientManager.isScheduler(pref.service)) {
						if (!pref.accounts.isEmpty()) {
							intent = new Intent(AutoAlbumNavigationGroup.this, AutoAlbumListActivity.class);
						}
					}
				}

				if (intent == null) {
					intent = new Intent(AutoAlbumNavigationGroup.this, OnlineSchedulerSetupActivity.class);
					JsMediaAuth auth = new JsMediaAuth(AutoAlbumNavigationGroup.this);
				...
				Toast.makeText(AutoAlbumNavigationGroup.this,
