...
package jp.co.johospace.jsphoto.onlineservice.gdata.api;
...
//		}
	...
	protected final <T> T executeGet(GoogleUrl url, Class<T> parseAsType)
			throws IOException {
	...
	protected final void executeDelete(GoogleUrl url, String etag)
			throws IOException {
		prepareUrl(url, null);
	...
	protected final <T> T executePost(GoogleUrl url,
			AbstractHttpContent content, Class<T> parseAsType)
			throws IOException {
		prepareUrl(url, parseAsType);
		HttpRequest request = getRequestFactory()
				.buildPostRequest(url, content);
		return execute(request).parseAs(parseAsType);
	}

	protected final <T> T executePut(GoogleUrl url,
			AbstractHttpContent content, Class<T> parseAsType)
			throws IOException {
		prepareUrl(url, parseAsType);
	...
	protected final <T> T executePatchRelativeToOriginal(GoogleUrl url,
			AbstractHttpContent patchContent, Class<T> parseAsType, String etag)
			throws IOException {
		prepareUrl(url, parseAsType);
		HttpRequest request = getRequestFactory().buildPatchRequest(url,
				patchContent);
		if (etag == null) {
